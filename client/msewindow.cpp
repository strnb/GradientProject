    ...
    , ui(new Ui::MSEWindow)
{
    ...
    connect(ui->calculateButton, &QPushButton::clicked, this, [this]() {
        ...
        std::stringstream sx(ui->xEdit->text().toStdString());
        std::stringstream sy(ui->yEdit->text().toStdString());

        double value;
        while (sx >> value) x.push_back(value);
        while (sy >> value) y.push_back(value);
        ...
        ui->resultLabel->setText(QString("MSE = %1").arg(mse));

        auto graph = findChild<MSEGraphWidget*>("graphWidget");
        if (graph) graph->setData(x, y, yPred);

        // Логируем на сервер
        NetworkManager::getInstance()->sendRequest(
            QString("CALC_MSE;%1;%2;%3")
                .arg(ui->xEdit->text(), ui->yEdit->text())
                .arg(mse)
        );
    });
